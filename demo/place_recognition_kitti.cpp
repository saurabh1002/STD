...
int main(int argc, char **argv) {
    ...
    std::cout << "Sucessfully load pose with number: " << poses_vec.size() << std::endl;
    ...
    while (ros::ok()) {
        ...
        lidar_data_path << lidar_path << std::setfill('0') << std::setw(6) << cloudInd << ".bin";
        ...
        if (cloudInd % config_setting.sub_frame_num_ == 0 && cloudInd != 0) {
            std::cout << "Key Frame id:" << keyCloudInd << ", cloud size: " << temp_cloud->size()
                      << std::endl;
            ...
            std::cout << "[Time] descriptor extraction: "
                      ...
                      << "update map:" << time_inc(t_map_update_end, t_map_update_begin) << "ms"
                      << std::endl;
            std::cout << std::endl;
    ...
    std::cout << "Mean time for descriptor extraction: " << mean_descriptor_time
              ...
              << "ms, total: " << mean_descriptor_time + mean_query_time + mean_update_time << "ms"
              << std::endl;
    return 0;
}
